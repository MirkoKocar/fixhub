...
export const OrnamentLine = ({opacity=0.15}) => (
  <div className="ornament-line" style={{display:'flex',alignItems:'center',gap:10,opacity}}>
...
export const StatusBadge = ({estado}) => {
  const map={nuevo:['var(--red)','Nuevo'],en_curso:['var(--yellow)','En curso'],resuelto:['var(--green)','Resuelto']}
  const [c,label]=map[estado]||['#A0AEC0',estado]
  return <span style={{fontSize:9,fontWeight:700,padding:'4px 11px',borderRadius:999,background:`color-mix(in srgb,${c} 12%,transparent)`,color:c,whiteSpace:'nowrap',letterSpacing:'0.04em',textTransform:'uppercase',border:`1px solid color-mix(in srgb,${c} 22%,transparent)`}}>{label}</span>
}

export const UrgenciaBadge = ({urgencia}) => {
  const map={baja:['var(--green)','Baja'],media:['var(--yellow)','Media'],alta:['var(--red)','Alta']}
  const [c,label]=map[urgencia]||['#A0AEC0',urgencia]
  return <span style={{fontSize:9,fontWeight:700,padding:'4px 11px',borderRadius:999,background:`color-mix(in srgb,${c} 10%,transparent)`,color:c,letterSpacing:'0.04em',textTransform:'uppercase'}}>{label}</span>
}

export const SkeletonCard = ({lines=2}) => (
  <div style={{background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:18,padding:'15px 16px'}}>
...
export const ActionBtn = ({label,sub,onClick,accent=false}) => (
  <div onClick={onClick} style={{
    ...
    <div style={{position:'absolute',top:0,left:14,right:14,height:1,background:'linear-gradient(to right,transparent,var(--gold-faint),transparent)'}}/>
...
export const BottomNav = ({rol='vecino'}) => {
  ...
  return (
    <nav style={{
      ...
      {/* indicador deslizante — dorado siempre */}
      <div style={{
        position:'absolute',top:5,height:3,borderRadius:999,
        ...
        {items.map(({path,label,Icon})=>{
          ...
          return (
            <button key={path} onClick={()=>navigate(path)} style={{
              ...
              // En light-mode el nav es navy oscuro → ícono activo dorado, inactivo blanco transparente
              color: active ? '#E0B05E' : 'rgba(255,255,255,0.35)',
            }}>
              <div style={{
                width:32,height:32,borderRadius:10,
                display:'flex',alignItems:'center',justifyContent:'center',
                background:active?'rgba(224,176,94,0.15)':'transparent',
