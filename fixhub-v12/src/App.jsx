...
// Eager load only critical paths
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {[0,1,2].map(i => (
          <div key={i} style={{
            ...
            background: 'rgba(224,176,94,0.6)',
            animation: `bounce 1.1s ${i*0.18}s ease-in-out infinite`
          }}/>
        ))}
      </div>
...
export default function App() {
  ...
  if (infraMode) return (
    <Suspense fallback={<PageLoader />}>
      <InfraDashboard onExit={() => setInfraMode(false)} />
    </Suspense>
  )
  if (showWelcome) return <Welcome onContinue={() => setShowWelcome(false)} />
  if (!session) return (<><PalaceFrame /><Login onLogin={handleLogin} /></>)
