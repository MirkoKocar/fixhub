    ...
    <div className="page">
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <Card style={{ padding: '16px' }}>
          ...
          <button onClick={() => setPreguntas([...preguntas, ''])} style={{ fontSize: 10, color: 'var(--border)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 12 }}>+ Agregar pregunta</button>
        ...
        {encuestas.map(e => (
          <Card key={e.id} style={{ padding: '14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              <p style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 500, flex: 1, marginRight: 10 }}>{e.titulo}</p>
              <span style={{ fontSize: 8.5, padding: '3px 8px', borderRadius: 2, background: e.activa ? 'rgba(52,211,153,0.1)' : 'var(--border)', color: e.activa ? '#34d399' : 'var(--text-faint)', border: `1px solid ${e.activa ? 'rgba(52,211,153,0.2)' : 'var(--border)'}`, whiteSpace: 'nowrap', flexShrink: 0 }}>
                {e.activa ? 'Activa' : 'Cerrada'}
              </span>
            </div>
            <p style={{ fontSize: 10, color: 'var(--text-faint)', marginBottom: 10 }}>{e.respuestas_encuesta?.length || 0} respuesta{e.respuestas_encuesta?.length !== 1 ? 's' : ''}</p>
