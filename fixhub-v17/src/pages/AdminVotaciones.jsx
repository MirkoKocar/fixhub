      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <Card style={{ padding: '16px' }}>
          ...
          <input value={titulo} onChange={e => setTitulo(e.target.value)} placeholder="Pregunta o tema a votar"
            style={{ width: '100%', background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 4, padding: '10px 14px', color: 'var(--text-primary)', fontSize: 13, marginBottom: 10 }} />
          ...
          {opciones.map((op, i) => (
            <input key={i} value={op} onChange={e => { const n = [...opciones]; n[i] = e.target.value; setOpciones(n) }} placeholder={`Opción ${i + 1}`}
              style={{ width: '100%', background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 4, padding: '9px 14px', color: 'var(--text-primary)', fontSize: 13, marginBottom: 6 }} />
          ))}
          <button onClick={() => setOpciones([...opciones, ''])} style={{ fontSize: 10, color: 'var(--border)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 10 }}>+ Agregar opción</button>
          <input type="date" value={fechaLimite} onChange={e => setFechaLimite(e.target.value)} min={new Date().toISOString().split('T')[0]}
            style={{ width: '100%', background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 4, padding: '9px 14px', color: 'var(--text-primary)', fontSize: 13, marginBottom: 12, colorScheme: 'dark' }} />
          ...
          return (
            <Card key={v.id} style={{ padding: '14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <p style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 500, flex: 1, marginRight: 10, lineHeight: 1.3 }}>{v.titulo}</p>
                <span style={{ fontSize: 8.5, padding: '3px 8px', borderRadius: 2, background: v.activa ? 'rgba(52,211,153,0.1)' : 'var(--border)', color: v.activa ? '#34d399' : 'var(--text-faint)', border: `1px solid ${v.activa ? 'rgba(52,211,153,0.2)' : 'var(--border)'}`, whiteSpace: 'nowrap', flexShrink: 0 }}>
                ...
                {conteo.map(({ op, count }) => (
                  <div key={op}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{op}</span>
                      <span style={{ fontSize: 11, color: 'var(--text-faint)' }}>{count} voto{count !== 1 ? 's' : ''}</span>
                    </div>
                    <div style={{ height: 3, background: 'var(--border)', borderRadius: 2 }}>
                      <div style={{ width: `${(count / max) * 100}%`, height: '100%', background: 'var(--border)', borderRadius: 2, transition: 'width 0.5s' }} />
                    </div>
                  </div>
                ))}
              </div>
              <p style={{ fontSize: 9, color: 'var(--text-faint)', marginTop: 10, letterSpacing: '0.06em' }}>{totalVotos} voto{totalVotos !== 1 ? 's' : ''} en total</p>
              {v.activa && (
