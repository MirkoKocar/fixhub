    ...
    <div className="page">
      ...
      <PageHeader title="Proveedores" subtitle="Gestión y rankings" onBack={() => navigate('/')} extra={
        <button onClick={() => setShowForm(!showForm)} style={{
          ...
          background: 'rgba(242,224,201,0.08)', border: '1px solid rgba(217,203,176,0.15)',
      ...
      {showForm && (
        <div style={{ padding: '0 20px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <Card style={{ padding: '16px' }}>
            <p className="font-cormorant" style={{ fontSize: 16, color: '#F2E0C9', marginBottom: 12 }}>Nuevo proveedor</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <input value={nombre} onChange={e => setNombre(e.target.value)} placeholder="Nombre completo"
                style={{ width: '100%', background: 'rgba(242,224,201,0.05)', border: '1px solid rgba(217,203,176,0.1)', borderRadius: 8, padding: '10px 14px', color: '#F2E0C9', fontSize: 14 }} />
              <select value={especialidad} onChange={e => setEspecialidad(e.target.value)}
                style={{ width: '100%', background: '#0B1A2E', border: '1px solid rgba(217,203,176,0.1)', borderRadius: 8, padding: '10px 14px', color: especialidad ? '#F2E0C9' : 'rgba(160,174,192,0.4)', fontSize: 14 }}>
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {loading ? (
          ...
          <Card key={p.id} style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(217,203,176,0.06)', border: '1px solid rgba(217,203,176,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <span className="font-cormorant" style={{ fontSize: 20, color: '#D9CBB0', fontWeight: 600 }}>{i + 1}</span>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontSize: 13, color: '#F2E0C9', fontWeight: 500 }}>{p.nombre}</p>
              <p style={{ fontSize: 10, color: 'rgba(160,174,192,0.35)', marginTop: 1 }}>{p.especialidad}</p>
              {/* Ranking bar — visible solo al admin */}
              <div style={{ marginTop: 5, display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ flex: 1, height: 3, background: 'rgba(217,203,176,0.08)', borderRadius: 2 }}>
                  <div style={{ width: `${p.ranking || 0}%`, height: '100%', background: 'rgba(217,203,176,0.4)', borderRadius: 2, transition: 'width 0.3s' }} />
                </div>
                <span style={{ fontSize: 9, color: 'rgba(160,174,192,0.3)' }}>{p.ranking || 0}</span>
              </div>
            </div>
