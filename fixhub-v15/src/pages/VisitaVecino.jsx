      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <Card style={{ padding: '14px' }}>
          ...
          <input value={nombre} onChange={e => setNombre(e.target.value)} placeholder="Nombre del visitante"
            style={{ width: '100%', background: 'rgba(242,224,201,0.04)', border: '1px solid rgba(217,203,176,0.1)', borderRadius: 4, padding: '10px 14px', color: '#F2E0C9', fontSize: 13, marginBottom: 7 }} />
          <input type="date" value={fecha} onChange={e => setFecha(e.target.value)} min={new Date().toISOString().split('T')[0]}
            style={{ width: '100%', background: 'rgba(242,224,201,0.04)', border: '1px solid rgba(217,203,176,0.1)', borderRadius: 4, padding: '10px 14px', color: '#F2E0C9', fontSize: 13, marginBottom: 7, colorScheme: 'dark' }} />
        ...
        {visitas.length === 0 ? (
          ...
          <AccentCard key={v.id} accentColor="rgba(217,203,176,0.18)">
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div>
                <p style={{ fontSize: 13, color: 'rgba(242,224,201,0.85)', fontWeight: 500 }}>{v.nombre_visita}</p>
                {v.nota && <p style={{ fontSize: 10, color: 'rgba(160,174,192,0.3)', marginTop: 2 }}>{v.nota}</p>}
              </div>
              <p style={{ fontSize: 10, color: 'rgba(160,174,192,0.3)', flexShrink: 0, marginLeft: 10 }}>
