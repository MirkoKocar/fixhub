...
function DiamondRating({ valor }) {
  ...
  return (
    <div style={{display:'flex',gap:4,alignItems:'center'}}>
      {[1,2,3,4,5].map(i=>(
        <div key={i} style={{width:6,height:6,transform:'rotate(45deg)',background:i<=stars?'rgba(224,176,94,0.7)':'transparent',border:`1px solid ${i<=stars?'rgba(224,176,94,0.5)':'rgba(217,203,176,0.15)'}`,transition:'all 0.2s'}}/>
      ))}
    </div>
  )
}
    ...
    <div className="page">
      ...
      <div style={{padding:'0 20px',display:'flex',flexDirection:'column',gap:12}}>
        ...
        <AccentCard accentColor="rgba(224,176,94,0.3)">
          ...
          <p style={{fontSize:14,color:'rgba(242,224,201,0.85)',fontWeight:500}}>{selected.especialidad}</p>
        </AccentCard>
        ...
        {selected.disponible ? (
          <div>
            <p style={{fontSize:11,color:'rgba(180,190,205,0.35)',textAlign:'center',marginBottom:14,lineHeight:1.6}}>
    ...
    <div className="page">
      ...
      <div style={{padding:'0 20px 18px',display:'flex',gap:7,overflowX:'auto'}}>
        {ESPECIALIDADES.map(e=>(
          <button key={e} onClick={()=>setFiltro(e)} style={{padding:'6px 14px',borderRadius:999,whiteSpace:'nowrap',flexShrink:0,background:filtro===e?'rgba(224,176,94,0.14)':'transparent',border:`1px solid ${filtro===e?'rgba(224,176,94,0.35)':'rgba(217,203,176,0.09)'}`,color:filtro===e?'#E0B05E':'rgba(180,190,205,0.3)',fontSize:10,fontWeight:700,letterSpacing:'0.06em',transition:'all 0.2s'}}>{e}</button>
        ))}
      </div>

      <div style={{padding:'0 20px',display:'flex',flexDirection:'column',gap:10}}>
        {loading ? (
          <p style={{textAlign:'center',color:'rgba(180,190,205,0.2)',fontSize:12,padding:'30px 0'}}>Cargando...</p>
        ) : filtrados.length === 0 ? (
          <Card style={{textAlign:'center',padding:'28px'}}>
            <p style={{fontSize:12,color:'rgba(180,190,205,0.28)'}}>Sin proveedores en esta categoría</p>
          </Card>
        ) : filtrados.map((p, i) => (
          <AccentCard key={p.id} onClick={() => setSelected(p)}
            accentColor={p.disponible?'rgba(52,211,153,0.35)':'rgba(217,203,176,0.12)'}>
            <div style={{display:'flex',gap:14,alignItems:'center'}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:2}}>
                  <p style={{fontSize:14,color:'rgba(242,224,201,0.88)',fontWeight:700}}>{p.nombre}</p>
                ...
                <p style={{fontSize:10,color:'rgba(180,190,205,0.35)',marginBottom:6,fontWeight:500}}>{p.especialidad}</p>
