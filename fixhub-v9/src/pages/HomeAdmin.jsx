...
function ActionBtn({ label, sub, onClick }) {
  return (
    <div onClick={onClick} style={{
      background:'rgba(255,255,255,0.03)', border:'1px solid rgba(217,203,176,0.1)',
      ...
      <p style={{ fontSize:12, fontWeight:700, color:'rgba(242,224,201,0.85)', fontFamily:"'DM Sans',sans-serif", lineHeight:1.2 }}>{label}</p>
  ...
  return (
    <div className="page page-enter">
      ...
      <div style={{ position:'relative', padding:'52px 20px 22px', textAlign:'center', overflow:'hidden' }}>
        ...
        <h1 className="font-serif" style={{ fontSize:24, color:'#F2E0C9', lineHeight:1.1 }}>Panel de Administración</h1>
        <p style={{ fontSize:9.5, color:'rgba(180,190,205,0.26)', letterSpacing:'0.1em', marginTop:4, fontWeight:500 }}>{user.edificio.nombre}</p>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:9 }}>
          {[{label:'Nuevos',value:stats.nuevo,color:'#f87171'},{label:'En curso',value:stats.en_curso,color:'#fbbf24'},{label:'Resueltos',value:stats.resuelto,color:'#34d399'}].map(s=>(
            <div key={s.label} style={{ background:`${s.color}07`, border:`1px solid ${s.color}18`, borderTop:`3px solid ${s.color}45`, borderRadius:16, padding:'14px 8px', textAlign:'center' }}>
              ...
              <p style={{ fontSize:8, color:'rgba(180,190,205,0.28)', letterSpacing:'0.1em', marginTop:5, textTransform:'uppercase', fontWeight:700 }}>{s.label}</p>
            </div>
          ))}
        </div>
        {stats.promedioDias > 0 && (
          <p style={{ fontSize:10, color:'rgba(180,190,205,0.28)', textAlign:'center', marginTop:8, fontWeight:500 }}>
            ...
            return (
              <AccentCard key={r.id} accentColor={color} style={{ marginBottom:7 }} onClick={() => navigate('/admin/recordatorios')}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <p style={{ fontSize:12, color:'rgba(242,224,201,0.82)', fontWeight:600 }}>{r.tipo}</p>
          ...
          {reservas.map(r => (
            <AccentCard key={r.id} accentColor="rgba(224,176,94,0.3)" style={{ marginBottom:7 }}>
              <div style={{ display:'flex', justifyContent:'space-between' }}>
                <div>
                  <p style={{ fontSize:12, color:'rgba(242,224,201,0.85)', fontWeight:700 }}>{r.espacio_nombre}</p>
                  <p style={{ fontSize:10, color:'rgba(180,190,205,0.3)', marginTop:2, fontWeight:500 }}>Depto {r.departamento} · {r.hora}</p>
                </div>
                <p style={{ fontSize:10, color:'rgba(180,190,205,0.32)', fontWeight:500 }}>{new Date(r.fecha).toLocaleDateString('es-AR',{day:'2-digit',month:'short'})}</p>
              </div>
            </AccentCard>
          ))}
        </div>
              ...
              <AccentCard key={a.id} onClick={() => navigate(`/admin/aviso/${a.id}`)}
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <div style={{ flex:1, minWidth:0, marginRight:10 }}>
                    <p style={{ fontSize:13, color:'rgba(242,224,201,0.9)', fontWeight:700, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{a.titulo}</p>
                    <p style={{ fontSize:9.5, color:'rgba(180,190,205,0.3)', marginTop:2, fontWeight:500 }}>{a.vecinos?.nombre} · Depto {a.vecinos?.departamento}</p>
                  </div>
