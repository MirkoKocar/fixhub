...
function ConfirmModal({ message, onConfirm, onCancel }) {
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', backdropFilter:'blur(6px)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div style={{ background:'#11203B', border:'1px solid rgba(248,113,113,0.25)', borderRadius:20, padding:24, width:'100%', maxWidth:380 }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={onCancel} style={{ flex:1, padding:'12px', borderRadius:999, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(217,203,176,0.15)', color:'rgba(242,224,201,0.6)', fontWeight:600, fontSize:13, cursor:'pointer' }}>Cancelar</button>
          <button onClick={onConfirm} style={{ flex:1, padding:'12px', borderRadius:999, background:'rgba(248,113,113,0.12)', border:'1px solid rgba(248,113,113,0.3)', color:'#f87171', fontWeight:700, fontSize:13, cursor:'pointer' }}>Eliminar</button>
        </div>
      </div>
    </div>
  )
...
export default function InfraDashboard({ onExit }) {
  ...
  const [confirm, setConfirm] = useState(null) // { type, id, nombre, extra }
  ...
  const handleDelete = async () => {
    ...
    if (type === 'vecino') {
      ...
      // Cascada completa: mensajes → avisos → vecinos → proveedores → anuncios → edificio
      const { data: avisosEdif } = await supabase.from('avisos').select('id').eq('edificio_id', id)
      if (avisosEdif?.length) {
        const ids = avisosEdif.map(a => a.id)
        await supabase.from('mensajes').delete().in('aviso_id', ids)
        await supabase.from('avisos').delete().in('id', ids)
      }
      await supabase.from('mensajes').delete().eq('edificio_id', id).catch(()=>{})
      await supabase.from('anuncios').delete().eq('edificio_id', id)
  ...
  const inputStyle = {
    ...
    border:'1px solid rgba(217,203,176,0.14)', borderRadius:12,
    padding:'11px 14px', color:'#F2E0C9', fontSize:13,
      ...
      <div style={{ padding:'20px', paddingBottom:40 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            {data.avisos.slice(0,5).map(a=>(
              <AccentCard key={a.id} accentColor={estadoColor[a.estado]||'#E0B05E'} style={{ marginBottom:7 }}>
                ...
                <p style={{ fontSize:9.5, color:'rgba(180,190,205,0.3)', marginTop:2 }}>{a.edificios?.nombre} · {a.vecinos?.nombre} · Depto {a.vecinos?.departamento}</p>
              </AccentCard>
            ))}
              ...
              return (
                <AccentCard key={e.id} accentColor="rgba(224,176,94,0.3)">
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                    ...
                    <button onClick={()=>setConfirm({type:'edificio',id:e.id,nombre:e.nombre})} style={{ padding:'6px', color:'rgba(248,113,113,0.4)', cursor:'pointer', flexShrink:0, marginLeft:8 }}>
            ...
            {data.vecinos.map(v=>(
              <AccentCard key={v.id} accentColor="rgba(96,165,250,0.3)">
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  ...
                  <button onClick={()=>setConfirm({type:'vecino',id:v.id,nombre:v.nombre})} style={{ padding:'6px', color:'rgba(248,113,113,0.4)', cursor:'pointer', flexShrink:0 }}>
                    <Trash2 size={14} strokeWidth={1.8}/>
                  </button>
                </div>
              </AccentCard>
            ...
            {data.proveedores.map(p=>(
              <AccentCard key={p.id} accentColor="rgba(52,211,153,0.3)">
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  ...
                  <button onClick={()=>setConfirm({type:'proveedor',id:p.id,nombre:p.nombre})} style={{ padding:'6px', color:'rgba(248,113,113,0.4)', cursor:'pointer', flexShrink:0 }}>
                    <Trash2 size={14} strokeWidth={1.8}/>
                  </button>
                </div>
              </AccentCard>
            ...
            {data.avisos.map(a=>(
              <AccentCard key={a.id} accentColor={estadoColor[a.estado]||'#E0B05E'}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  ...
                  <button onClick={()=>setConfirm({type:'aviso',id:a.id,nombre:a.titulo})} style={{ padding:'6px', color:'rgba(248,113,113,0.4)', cursor:'pointer', flexShrink:0 }}>
                    <Trash2 size={14} strokeWidth={1.8}/>
                  </button>
                </div>
              </AccentCard>
        ...
        {!loading && tab === 'stats' && (
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            {avisosPorEdificio.length===0 && <p style={{ fontSize:12, color:'rgba(180,190,205,0.2)', textAlign:'center', padding:'20px 0' }}>Sin datos todavía.</p>}
