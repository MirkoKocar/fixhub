...
export default function Chat({ user }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100dvh', background:'#0A1428', maxWidth:430, margin:'0 auto' }}>
      ...
      <div style={{ padding:'44px 18px 12px', background:'rgba(10,20,40,0.97)', backdropFilter:'blur(20px)', borderBottom:'1px solid rgba(217,203,176,0.07)', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
          <div style={{ flex:1, minWidth:0 }}>
            <h2 className="font-serif" style={{ fontSize:17, color:'#F2E0C9', lineHeight:1.2, marginBottom:2 }}>{aviso?.titulo}</h2>
          ...
          {user.rol === 'admin' && (
            <select value={aviso?.estado} onChange={e => cambiarEstado(e.target.value)}
              style={{ marginLeft:10, background:'#11203B', border:'1px solid rgba(217,203,176,0.12)', borderRadius:10, padding:'6px 10px', color:'#F2E0C9', fontSize:10, fontWeight:600, flexShrink:0 }}>
          ...
          return (
            <div key={m.id || i} style={{ display:'flex', justifyContent:m.es_nota_interna?'center':propio?'flex-end':'flex-start' }}>
              {m.es_nota_interna ? (
                ...
                <div style={{ maxWidth:'76%', padding:'10px 14px', borderRadius:16, borderBottomRightRadius:propio?4:16, borderBottomLeftRadius:propio?16:4, background:propio?'rgba(224,176,94,0.12)':'rgba(255,255,255,0.04)', border:`1px solid ${propio?'rgba(224,176,94,0.2)':'rgba(217,203,176,0.07)'}`, opacity:m._temp?0.7:1, transition:'opacity 0.2s' }}>
                  <p style={{ fontSize:14, color:propio?'#F2E0C9':'rgba(242,224,201,0.75)', lineHeight:1.45, fontWeight:500 }}>{m.contenido}</p>
                  <div style={{ display:'flex', justifyContent:propio?'flex-end':'flex-start', alignItems:'center', gap:4, marginTop:3 }}>
                    <p style={{ fontSize:9, color:'rgba(180,190,205,0.22)' }}>{new Date(m.created_at).toLocaleTimeString('es-AR', { hour:'2-digit', minute:'2-digit' })}</p>
                    {propio && !m._temp && <Check size={10} color="rgba(180,190,205,0.3)" strokeWidth={2}/>}
                  </div>
                </div>
              )}
            </div>
      ...
      {user.rol === 'proveedor' && (
        <div style={{ padding:'8px 16px 0', display:'flex', gap:7, overflowX:'auto', flexShrink:0 }}>
          {RESPUESTAS_RAPIDAS.map(r => (
            <button key={r} onClick={() => sendMsg(r)} style={{ padding:'6px 14px', borderRadius:999, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(217,203,176,0.1)', color:'rgba(217,203,176,0.55)', fontSize:10, whiteSpace:'nowrap', fontWeight:600, flexShrink:0 }}>{r}</button>
          ))}
        </div>
      )}
      ...
      {/* Input */}
      <div style={{ padding:'10px 15px 20px', background:'rgba(10,20,40,0.97)', backdropFilter:'blur(20px)', borderTop:'1px solid rgba(217,203,176,0.07)', display:'flex', gap:8, alignItems:'flex-end', flexShrink:0 }}>
        {user.rol === 'admin' && (
          <button onClick={() => setShowNota(!showNota)} style={{ width:38, height:38, borderRadius:12, background:showNota?'rgba(251,191,36,0.12)':'rgba(255,255,255,0.04)', border:`1px solid ${showNota?'rgba(251,191,36,0.25)':'rgba(217,203,176,0.1)'}`, color:showNota?'rgba(251,191,36,0.7)':'rgba(180,190,205,0.3)', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:16 }}>✎</button>
        )}
        ...
        <button onClick={() => sendMsg()} disabled={!texto.trim()} style={{ width:40, height:40, borderRadius:13, flexShrink:0, background:texto.trim()?'linear-gradient(135deg,#E0B05E,#C9923A)':'rgba(255,255,255,0.05)', border:'1px solid rgba(217,203,176,0.1)', color:texto.trim()?'#0A1428':'rgba(180,190,205,0.25)', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:texto.trim()?'0 2px 12px rgba(224,176,94,0.28)':'none', transition:'all 0.2s' }}>
