...
export default function InfraDashboard({ onExit }) {
  ...
  const safeDelete = (table, col, val) =>
    supabase.from(table).delete().eq(col, val).then(() => {}).catch(() => {})

  const safeDeleteIn = (table, col, vals) =>
    vals?.length ? supabase.from(table).delete().in(col, vals).then(() => {}).catch(() => {}) : Promise.resolve()
  ...
  const handleDelete = async () => {
    ...
    try {
      if (type === 'vecino') {
        ...
        // Borrar mensajes por aviso, vecino y proveedor
        ...
        // Borrar dependencias de vecinos
        ...
        // Borrar dependencias de proveedores
        await safeDeleteIn('agenda_proveedor','proveedor_id', provIds)

        // Borrar tablas propias del edificio
