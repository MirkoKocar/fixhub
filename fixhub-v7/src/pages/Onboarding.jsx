  ...
  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', padding: '48px 32px 36px', background: '#0A1428', maxWidth: 430, margin: '0 auto' }}>
      <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-end' }}>
        <button onClick={onFinish} style={{ fontSize: 10, color: 'rgba(180,190,205,0.28)', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '6px 14px', borderRadius: 999, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(217,203,176,0.1)' }}>Saltar</button>
      </div>
      ...
      <div key={paso} className="fade-up" style={{ textAlign: 'center', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <p style={{ fontSize: 13, color: 'rgba(180,190,205,0.45)', lineHeight: 1.7, maxWidth: 280 }}>{actual.desc}</p>
      </div>

      <div style={{ width: '100%' }}>
        <div style={{ display: 'flex', justifyContent: 'center', gap: 10, marginBottom: 20 }}>
          {pasos.map((_, i) => (
            <div key={i} style={{ width: i === paso ? 20 : 6, height: 6, borderRadius: 999, background: i === paso ? 'linear-gradient(90deg,#E0B05E,#D49A45)' : 'rgba(217,203,176,0.15)', transition: 'all 0.3s', boxShadow: i === paso ? '0 0 8px rgba(224,176,94,0.4)' : 'none' }} />
          ))}
        </div>
        {paso < pasos.length - 1 ? (
