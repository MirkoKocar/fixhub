...
export const PalaceFrame = () => <div className="blob-bg" />
...
export const OrnamentLine = ({ opacity=0.16 }) => (
  <div style={{display:'flex',alignItems:'center',gap:10,opacity}}>
    ...
    <span className="font-cormorant" style={{fontSize:8,color:'#E0B05E',letterSpacing:'0.3em'}}>✦</span>
...
export const SectionLabel = ({children,style={}}) => (
  <p style={{fontSize:8.5,letterSpacing:'0.3em',color:'rgba(180,190,205,0.32)',textTransform:'uppercase',...style}}>{children}</p>
)

// Card — rounded, soft
export const Card = ({children,style={},onClick}) => (
  <div onClick={onClick} style={{
    background:'rgba(255,255,255,0.025)',border:'1px solid rgba(217,203,176,0.1)',
    borderRadius:16,padding:'15px 16px',cursor:onClick?'pointer':'default',...style
  }}>{children}</div>
)

export const AccentCard = ({children,style={},onClick,accentColor='rgba(224,176,94,0.3)'}) => (
  <div onClick={onClick} style={{
    background:'rgba(255,255,255,0.02)',
    border:'1px solid rgba(217,203,176,0.08)',
    borderLeft:`3px solid ${accentColor}`,
    borderRadius:14,padding:'13px 16px',cursor:onClick?'pointer':'default',...style
  }}>{children}</div>
)
...
export const PrimaryBtn = ({children,onClick,disabled,style={}}) => (
  <button onClick={onClick} disabled={disabled} style={{
    width:'100%',
    background: disabled ? 'rgba(224,176,94,0.3)' : 'linear-gradient(135deg, #E0B05E 0%, #D49A45 100%)',
    border:'none',borderRadius:999,padding:'15px',
    fontFamily:"'Inter',sans-serif",fontSize:14,fontWeight:700,
    color:'#0A1428',letterSpacing:'0.04em',
    boxShadow: disabled ? 'none' : '0 4px 16px rgba(224,176,94,0.25)',
...
export const GhostBtn = ({children,onClick,style={}}) => (
  <button onClick={onClick} style={{
    ...
    color:'rgba(242,224,201,0.6)', letterSpacing:'0.03em',
    ...style
  }}>{children}</button>
)
...
export const StatusBadge = ({estado}) => {
  ...
  return <span style={{fontSize:8.5,fontWeight:700,padding:'4px 10px',borderRadius:999,background:`${c}15`,color:c,whiteSpace:'nowrap',letterSpacing:'0.06em',textTransform:'uppercase',border:`1px solid ${c}25`}}>{label}</span>
}
...
export const UrgenciaBadge = ({urgencia}) => {
  ...
  return <span style={{fontSize:8.5,fontWeight:700,padding:'4px 10px',borderRadius:999,background:`${c}12`,color:c,letterSpacing:'0.06em',textTransform:'uppercase'}}>{label}</span>
}

// Filled icon container — more body, less wireframe
export const OrnIcon = ({path,size=18,color='#E0B05E',bg,style={}}) => (
  <div style={{
    width:42,height:42,flexShrink:0,borderRadius:13,
    display:'flex',alignItems:'center',justifyContent:'center',
    background: bg || 'linear-gradient(135deg, rgba(224,176,94,0.18) 0%, rgba(224,176,94,0.06) 100%)',
    border:'1px solid rgba(224,176,94,0.15)',
    ...style
  }}>
    <svg width={size} height={size} fill="none" stroke={color} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
      <path d={path}/>
    </svg>
  </div>
)
...
export const SkeletonCard = ({lines=2}) => (
  <div style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(217,203,176,0.08)',borderRadius:16,padding:'15px 16px'}}>
...
export const NotifBanner = ({message,onClose}) => (
  <div className="slide-down" style={{
    ...
    borderBottom:'1px solid rgba(224,176,94,0.18)',
    display:'flex',justifyContent:'space-between',alignItems:'center'
  }}>
    <div style={{display:'flex',alignItems:'center',gap:10}}>
      <div style={{width:8,height:8,borderRadius:'50%',background:'#E0B05E',flexShrink:0,boxShadow:'0 0 8px rgba(224,176,94,0.6)'}}/>
      <p style={{fontSize:12,color:'rgba(242,224,201,0.85)',letterSpacing:'0.02em'}}>{message}</p>
    </div>
    <button onClick={onClose} style={{fontSize:16,color:'rgba(180,190,205,0.4)',padding:'0 4px'}}>×</button>
  </div>
)
...
export const PageHeader = ({title,subtitle,onBack,extra}) => (
  <div style={{padding:'44px 20px 18px',textAlign:'center',position:'relative'}}>
    {onBack&&(
      <button onClick={onBack} style={{position:'absolute',left:16,top:48,color:'rgba(180,190,205,0.35)',fontSize:10,letterSpacing:'0.08em',display:'flex',alignItems:'center',gap:4,textTransform:'uppercase',background:'rgba(255,255,255,0.03)',padding:'6px 12px',borderRadius:999,border:'1px solid rgba(217,203,176,0.1)'}}>
        <svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7"/></svg>
        Volver
      </button>
    )}
    <p className="font-cormorant animate-ornament" style={{color:'#E0B05E',fontSize:9,letterSpacing:'0.5em',marginBottom:10}}>✦ ✦ ✦</p>
    <h1 className="font-cormorant" style={{fontSize:25,fontWeight:700,color:'#F2E0C9',letterSpacing:'0.01em',lineHeight:1.1,marginBottom:subtitle?3:0}}>{title}</h1>
    {subtitle&&<p style={{fontSize:10,color:'rgba(180,190,205,0.35)',letterSpacing:'0.08em'}}>{subtitle}</p>}
...
export const BottomNav = ({rol='vecino'}) => {
  ...
  useEffect(() => {
    ...
    if (btn) {
      setIndicatorStyle({
        left: btn.offsetLeft + btn.offsetWidth / 2 - 14,
        width: 28,
        opacity: 1
      })
    }
  ...
  return (
    <nav style={{position:'fixed',bottom:0,left:'50%',transform:'translateX(-50%)',width:'100%',maxWidth:430,background:'rgba(10,20,40,0.97)',backdropFilter:'blur(24px)',borderTop:'1px solid rgba(217,203,176,0.08)',padding:'10px 0 24px',zIndex:40}}>
      {/* Sliding indicator */}
      <div style={{
        position:'absolute', top:6, height:2.5, borderRadius:2,
        background:'linear-gradient(90deg, #E0B05E, #D49A45)',
        transition:'left 0.35s cubic-bezier(.22,1,.36,1), width 0.35s cubic-bezier(.22,1,.36,1)',
        boxShadow: '0 0 8px rgba(224,176,94,0.5)',
        ...indicatorStyle
      }}/>
      <div ref={containerRef} style={{display:'flex',justifyContent:'space-around'}}>
        {items.map(item=>{
          ...
          return (
            <button key={item.path} onClick={()=>navigate(item.path)} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:4,color:active?'#F2E0C9':'rgba(180,190,205,0.28)',padding:'4px 10px',minWidth:46}}>
              <svg width="18" height="18" fill={active ? 'rgba(224,176,94,0.12)' : 'none'} stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d={item.d}/></svg>
              <span style={{fontSize:7.5,fontWeight:600,letterSpacing:'0.08em',textTransform:'uppercase'}}>{item.label}</span>
            </button>
          )
        })}
      </div>
