...
import { PalaceFrame, OrnamentLine, DiamondRow, Card, AccentCard, StatusBadge, BottomNav, SectionLabel, SkeletonCard, NotifBanner } from '../components/Palace'
...
// Botones de acción sin iconos con color — estilo tipográfico refinado
function ActionBtn({ label, sub, onClick }) {
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{ position:'absolute', top:0, left:16, right:16, height:1, background:'linear-gradient(to right,transparent,rgba(224,176,94,0.2),transparent)' }}/>
      <p style={{ fontSize:13, fontWeight:700, color:'rgba(242,224,201,0.88)', lineHeight:1.2, fontFamily:"'DM Sans',sans-serif" }}>{label}</p>
      <p style={{ fontSize:9.5, color:'rgba(180,190,205,0.3)', fontWeight:500, letterSpacing:'0.04em' }}>{sub}</p>
    </div>
  )
}
...
export default function HomeVecino({ user }) {
  ...
  return (
    <div className="page page-enter">
        ...
        <div style={{ padding:'0 20px 16px' }}>
          ...
          {anuncios.map(a => (
            <AccentCard key={a.id} accentColor={a.prioridad==='urgente'?'#f87171':'rgba(224,176,94,0.4)'} style={{ marginBottom:8 }}>
              ...
              <p style={{ fontSize:11, color:'rgba(180,190,205,0.42)', marginTop:4, lineHeight:1.5, fontWeight:400 }}>{a.contenido}</p>
            </AccentCard>
          ))}
        </div>
      )}
      ...
      {/* Acceso rápido — texto puro, sin iconos chillones */}
      <div style={{ padding:'0 20px 20px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
          ...
          <ActionBtn label="Servicios" sub="Ver proveedores" onClick={() => navigate('/proveedores')} />
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
            {emergencias.map(e => (
              <a key={e.id} href={`tel:${e.telefono}`} style={{ display:'block', textDecoration:'none', background:'rgba(248,113,113,0.05)', border:'1px solid rgba(248,113,113,0.14)', borderRadius:14, padding:'12px 14px' }}>
                ...
                <p style={{ fontSize:14, color:'#f87171', fontFamily:"'DM Serif Display',serif", fontWeight:400 }}>{e.telefono}</p>
              </a>
            ))}
          </div>
        </div>
      ...
      <div style={{ padding:'12px 20px 0' }}>
        ...
        {loading ? (
          ...
          <Card style={{ textAlign:'center', padding:'22px', borderRadius:18 }}>
