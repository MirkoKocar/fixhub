...
export default function AdminTablon({ user }) {
  ...
  const colors = { urgente: '#f87171', normal: 'rgba(217,203,176,0.3)', info: '#60a5fa' }
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <Card style={{ padding: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
            {['info','normal','urgente'].map(p => (
              <button key={p} onClick={() => setPrioridad(p)} style={{
                ...
                border: `1px solid ${prioridad === p ? colors[p] + '35' : 'rgba(217,203,176,0.08)'}`,
                color: prioridad === p ? colors[p] : 'rgba(160,174,192,0.3)',
          ...
          <textarea value={contenido} onChange={e => setContenido(e.target.value)} placeholder="Contenido del anuncio..." rows={3}
            style={{ width: '100%', background: 'rgba(242,224,201,0.04)', border: '1px solid rgba(217,203,176,0.1)', borderRadius: 4, padding: '10px 14px', color: '#F2E0C9', fontSize: 13, resize: 'none', lineHeight: 1.5, marginBottom: 12 }} />
        ...
        {anuncios.length === 0 ? (
          <Card style={{ textAlign: 'center', padding: '24px' }}>
            <p style={{ fontSize: 12, color: 'rgba(160,174,192,0.28)' }}>Sin anuncios publicados</p>
          </Card>
        ) : (
          anuncios.map(a => (
            <AccentCard key={a.id} accentColor={colors[a.prioridad] || colors.normal}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 13, color: '#F2E0C9', fontWeight: 500 }}>{a.titulo}</p>
                  <p style={{ fontSize: 11, color: 'rgba(160,174,192,0.4)', marginTop: 4, lineHeight: 1.5 }}>{a.contenido}</p>
                  <p style={{ fontSize: 9, color: 'rgba(160,174,192,0.2)', marginTop: 6, letterSpacing: '0.06em' }}>
