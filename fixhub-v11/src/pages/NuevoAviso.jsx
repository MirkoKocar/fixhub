              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                {CATEGORIAS.map(c => (
                  <button key={c} onClick={() => setCategoria(c)} style={{
                    ...
                    background: categoria === c ? 'rgba(224,176,94,0.14)' : 'rgba(255,255,255,0.03)',
                    border: `1px solid ${categoria === c ? 'rgba(224,176,94,0.4)' : 'rgba(217,203,176,0.1)'}`,
                    color: categoria === c ? '#E0B05E' : 'rgba(180,190,205,0.45)',
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {URGENCIAS.map(u => (
                  <button key={u.value} onClick={() => setUrgencia(u.value)} style={{
                    ...
                    background: urgencia === u.value ? `${u.color}0C` : 'rgba(255,255,255,0.02)',
                    ...
                    <p style={{ fontSize: 12, fontWeight: 700, color: urgencia === u.value ? u.color : 'rgba(180,190,205,0.35)' }}>{u.label}</p>
                    <p style={{ fontSize: 9, color: 'rgba(180,190,205,0.25)', marginTop: 2 }}>{u.desc}</p>
                  </button>
                ))}
              </div>
            </div>
