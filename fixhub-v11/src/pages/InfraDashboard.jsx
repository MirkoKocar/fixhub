...
import { Building2, Users, Wrench, ClipboardList, Trash2, LogOut, AlertCircle } from 'lucide-react'

function ConfirmModal({ message, onConfirm, onCancel }) {
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', backdropFilter:'blur(6px)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:24, maxWidth:430, margin:'0 auto' }}>
...
export default function InfraDashboard({ onExit }) {
  ...
  const fetchAll = async () => {
    ...
    const [edRes, vRes, pRes, avRes] = await Promise.all([
      ...
      supabase.from('avisos').select('id, titulo, estado, edificio_id, vecino_id, edificios(nombre), vecinos(nombre,departamento)').order('created_at', { ascending:false }).limit(100),
    ])
    setData({
  ...
  const addEdificio = async () => {
    ...
    if (!nombre || !codigo) {
      setMsg({ text:'Completá al menos el nombre y el código de acceso.', ok:false })
      return
    }
    ...
    if (error) {
      ...
      setMsg({ text: isDupe ? 'Ese código de acceso ya existe. Usá uno diferente.' : `Error: ${error.message}`, ok:false })
      setSaving(false)
      return
    }
    ...
    if (ed) {
      ...
      setMsg({ text:`Edificio "${ed.nombre}" agregado con código ${ed.codigo_acceso}.`, ok:true })
  ...
  const handleDelete = async () => {
    ...
    if (type === 'vecino') {
      ...
      setData(prev => ({ ...prev, vecinos:prev.vecinos.filter(v => v.id !== id) }))
      ...
      setData(prev => ({ ...prev, edificios:prev.edificios.filter(e => e.id !== id) }))
    }
    setConfirm(null)
  }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#060D1C', fontFamily:"'DM Sans',sans-serif", maxWidth:430, margin:'0 auto', position:'relative' }}>
      ...
      <div style={{ padding:'48px 20px 16px', textAlign:'center', borderBottom:'1px solid rgba(224,176,94,0.1)' }}>
        ...
        <button onClick={onExit} style={{ marginTop:10, fontSize:9, color:'rgba(248,113,113,0.55)', letterSpacing:'0.1em', textTransform:'uppercase', fontWeight:700, padding:'6px 16px', border:'1px solid rgba(248,113,113,0.2)', borderRadius:999, display:'inline-flex', alignItems:'center', gap:6, cursor:'pointer', background:'none' }}>
        ...
        {!loading && tab === 'edificios' && (
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            <Card style={{ padding:'16px' }}>
              ...
              <input
                value={newEdificio.nombre}
                onChange={e => setNewEdificio(prev=>({...prev, nombre:e.target.value}))}
                placeholder="Nombre del edificio *"
                style={inputStyle}
              />
              <input
                value={newEdificio.direccion}
                onChange={e => setNewEdificio(prev=>({...prev, direccion:e.target.value}))}
                placeholder="Dirección (opcional)"
                style={inputStyle}
              />
              <input
                value={newEdificio.codigo_acceso}
                onChange={e => setNewEdificio(prev=>({...prev, codigo_acceso:e.target.value.toUpperCase()}))}
                placeholder="Código de acceso * (ej: EDIF02)"
              ...
              <PrimaryBtn onClick={addEdificio} disabled={saving}>
                {saving ? 'Guardando...' : 'Agregar edificio'}
              </PrimaryBtn>
            </Card>
            ...
            {data.edificios.length === 0 && (
              <p style={{ fontSize:12, color:'rgba(180,190,205,0.2)', textAlign:'center', padding:'20px 0' }}>Todavía no hay edificios. ¡Agregá el primero!</p>
            )}
                ...
                <AccentCard key={e.id} accentColor="rgba(224,176,94,0.3)">
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                    <div style={{ flex:1 }}>
                      ...
                      {e.direccion && <p style={{ fontSize:10, color:'rgba(180,190,205,0.35)', marginTop:2, fontWeight:500 }}>{e.direccion}</p>}
                      <div style={{ display:'flex', gap:12, marginTop:6 }}>
                        ...
                        <span style={{ fontSize:9.5, color:'rgba(224,176,94,0.7)', fontWeight:700, fontFamily:"'DM Serif Display',serif", letterSpacing:'0.05em' }}>{e.codigo_acceso}</span>
                      </div>
                    </div>
