        ...
        {step === 1 && (
          <div>
            ...
            <div style={{ position: 'relative', marginBottom: 10 }}>
              ...
              />
              <button onClick={handleCode} disabled={loading} style={{ position: 'absolute', right: 5, top: '50%', transform: 'translateY(-50%)', width: 38, height: 38, borderRadius: '50%', background: input.trim() ? 'linear-gradient(135deg,#E0B05E,#C9923A)' : 'rgba(255,255,255,0.05)', border: 'none', color: input.trim() ? '#0A1428' : 'rgba(180,190,205,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: input.trim() ? '0 2px 10px rgba(224,176,94,0.3)' : 'none' }}>
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }} className="scale-in">
            <div style={{ padding: '13px 16px', background: `linear-gradient(135deg,${rolColors[parsed?.rol]}0D,transparent)`, border: `1px solid ${rolColors[parsed?.rol]}22`, borderRadius: 18, textAlign: 'center', marginBottom: 2 }}>
              ...
              <p style={{ fontSize: 10, color: 'rgba(180,190,205,0.32)', marginTop: 2, fontWeight: 500 }}>{edificio?.direccion}</p>
            </div>
