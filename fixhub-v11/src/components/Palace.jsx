...
export const NotifBanner = ({message,onClose}) => (
  <div className="slide-down" style={{
    ...
    width:'100%',maxWidth:430,zIndex:200,padding:'12px 20px',
  ...
  <div style={{padding:'44px 20px 18px',textAlign:'center',position:'relative'}}>
    {onBack&&(
      <button onClick={onBack} style={{
        ...
        background:'var(--bg3)',padding:'7px 14px',borderRadius:999,
        border:'1px solid var(--border)',fontWeight:600
      }}>
        <ChevronLeft size={13}/> Volver
      </button>
    )}
    <p className="animate-ornament" style={{color:'var(--gold)',fontSize:9,letterSpacing:'0.5em',marginBottom:10,opacity:0.55}}>✦ ✦ ✦</p>
...
export const ActionBtn = ({label,sub,onClick,accent=false}) => (
  <div onClick={onClick} style={{
    ...
    border:`1px solid ${accent?'rgba(224,176,94,0.22)':'var(--border)'}`,
...
export const BottomNav = ({rol='vecino'}) => {
  ...
  return (
    <nav style={{
      ...
      paddingTop:10,paddingBottom:'max(20px, env(safe-area-inset-bottom))',zIndex:40
    }}>
        ...
        {items.map(({path,label,Icon})=>{
          ...
          return (
            <button key={path} onClick={()=>navigate(path)} style={{
              ...
              <div style={{width:32,height:32,borderRadius:10,display:'flex',alignItems:'center',justifyContent:'center',background:active?'var(--gold-faint)':'transparent',transition:'background 0.2s'}}>
