...
function ElegirProveedor({ edificioId, categoria, onSelect }) {
  ...
  if (loading) return <p style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '30px 0', fontSize: 12 }}>Buscando proveedores...</p>

  if (proveedores.length === 0) return (
    <Card style={{ textAlign: 'center', padding: '24px' }}>
      <p style={{ fontSize: 13, color: 'var(--text-muted)', fontWeight: 500 }}>No hay proveedores disponibles para {categoria}.</p>
      <p style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 6 }}>El administrador recibirá tu aviso y asignará uno.</p>
    </Card>
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <p style={{ fontSize: 10, color: 'var(--text-faint)', marginBottom: 6, fontWeight: 400 }}>Ordenados por preferencia del edificio. No se muestran datos de contacto.</p>
      {proveedores.map(p => (
        <AccentCard key={p.id} accentColor={p.disponible ? 'rgba(52,211,153,0.4)' : 'var(--border)'} onClick={() => p.disponible && onSelect(p)}
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <>
            ...
            <div>
              <SectionLabel style={{ marginBottom: 8 }}>
                Descripción {esOtro ? <span style={{ color: '#f87171', fontWeight: 700 }}> *obligatorio</span> : <span style={{ color: 'var(--text-faint)', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(opcional)</span>}
              </SectionLabel>
        ...
        {step === 2 && avisoCreado && (
          <>
            ...
            <OrnamentLine opacity={0.08}/>
            <button onClick={handleSinProveedor} style={{ fontSize: 11, color: 'var(--text-faint)', textAlign: 'center', padding: '8px', fontWeight: 600, letterSpacing: '0.06em' }}>
