...
export default function Login({ onLogin }) {
  ...
  const handleCode = async () => {
    ...
    // INFRA secret mode
    if (p.rol === 'infra') { onLogin({ _infra: true }); return }
  ...
  const inputStyle = { width: '100%', background: 'var(--input-bg)', border: '1px solid var(--input-border)', borderRadius: 999, padding: '13px 20px', color: 'var(--text-primary)', fontSize: 14, fontFamily: "'DM Sans',sans-serif", fontWeight: 500 }
        ...
        {step === 1 && (
          <div>
            ...
            <div style={{ position: 'relative', marginBottom: 10 }}>
              ...
              />
              <button onClick={handleCode} disabled={loading} style={{ position: 'absolute', right: 5, top: '50%', transform: 'translateY(-50%)', width: 38, height: 38, borderRadius: '50%', background: input.trim() ? 'linear-gradient(135deg,#E0B05E,#C9923A)' : 'var(--bg-card)', border: 'none', color: input.trim() ? '#0A1428' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: input.trim() ? '0 2px 10px rgba(224,176,94,0.3)' : 'none' }}>
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }} className="scale-in">
            <div style={{ padding: '13px 16px', background: `linear-gradient(135deg,${rolColors[parsed?.rol]}0D,transparent)`, border: `1px solid ${rolColors[parsed?.rol]}22`, borderRadius: 18, textAlign: 'center', marginBottom: 2 }}>
              ...
              <p style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2, fontWeight: 500 }}>{edificio?.direccion}</p>
            </div>
            ...
            <button onClick={() => { setStep(1); setError(''); setParsed(null); setEdificio(null) }} style={{ color: 'var(--text-faint)', fontSize: 10, padding: '6px', letterSpacing: '0.1em', textTransform: 'uppercase', textAlign: 'center', fontWeight: 600 }}>← Volver</button>
          </div>
        )}
      </div>
