    ...
    <div className="page page-enter">
      ...
      <div style={{ padding:'0 20px', display:'flex', flexDirection:'column', gap:12 }}>
        ...
        <AccentCard accentColor={lightMode?'rgba(251,191,36,0.5)':'rgba(96,165,250,0.4)'}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              {lightMode ? <Sun size={16} color="#fbbf24" strokeWidth={1.5}/> : <Moon size={16} color="#60a5fa" strokeWidth={1.5}/>}
              <div>
                <p style={{ fontSize:13, color:'rgba(242,224,201,0.88)', fontWeight:700 }}>Modo {lightMode?'claro':'oscuro'}</p>
                <p style={{ fontSize:10, color:'rgba(180,190,205,0.32)', marginTop:2, fontWeight:500 }}>Cambiar tema visual</p>
              </div>
            </div>
        ...
        <Card>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <input type="range" min="50" max="120" value={brightness} onChange={e => setBrightness(e.target.value)}
              style={{ flex:1, accentColor:'#E0B05E', cursor:'pointer' }}/>
        ...
        <AccentCard accentColor={notifOn?'rgba(52,211,153,0.4)':'rgba(217,203,176,0.15)'}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              {notifOn ? <Bell size={16} color="#34d399" strokeWidth={1.5}/> : <BellOff size={16} color="rgba(180,190,205,0.4)" strokeWidth={1.5}/>}
              <div>
                <p style={{ fontSize:13, color:'rgba(242,224,201,0.88)', fontWeight:700 }}>Notificaciones</p>
        ...
        <Card>
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {[['Versión','FixHub v10.0'],['Edificio',user.edificio?.nombre],['Código',user.edificio?.codigo_acceso]].map(([label,val]) => (
        ...
        <button onClick={handleLogout} style={{ width:'100%', background:'rgba(248,113,113,0.07)', border:'1px solid rgba(248,113,113,0.2)', borderRadius:999, padding:'14px', fontSize:14, fontWeight:700, color:'#f87171', marginTop:8, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
